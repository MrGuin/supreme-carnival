...
type LogEntry struct {
	...
	Index   int         // entry's index in the log
}
...
type Raft struct {
	...
	applyCh chan ApplyMsg // channel to notify tester after applying log entries
...
func (rf *Raft) Start(command interface{}) (int, int, bool) {
	...
	defer rf.mu.Unlock()
	term = rf.currentTerm
	if rf.state != leader { // current server is not leader
		isLeader = false
...
func Make(peers []*labrpc.ClientEnd, me int,
	...
	go func() {
		...
		defer rf.mu.Unlock()
		for !rf.killed() {
			for rf.commitIndex <= rf.lastApplied { // wait until there are new entries to commit
				rf.applyCond.Wait()
			}
